...
docker exec -i postgres_container psql -U %PG_USER% -d %PG_DATABASE% -f /tmp/populate.sql
IF %ERRORLEVEL% NEQ 0 (
    echo ❌ Failed to populate the database.
    docker logs postgres_container --tail 120
    pause
    exit /b 1
)
...
echo ----------------------------------------
echo [STEP 4] Checking seed result (Items count)...

docker exec -i %CONTAINER_NAME% psql -U %PG_USER% -d %PG_DATABASE% -c "SELECT COUNT(*) AS ""ItemsCount"" FROM ""Items"";"
IF %ERRORLEVEL% NEQ 0 (
    echo ⚠️ Could not query Items table.
    echo Showing last logs:
